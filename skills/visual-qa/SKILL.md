...
|---------|--------------|----------|
| Element invisible in dark mode | Hardcoded light color | Use a local token such as `var(--md-sys-color-*)` |
...
1. **Local component issue** - Edit the component in `components/ui/` and run `npm run build:components`
2. **App-specific CSS issue** - Edit `styles.css`
...
## Integration with Local Components

When issues are found in `wy-*` web components:

1. The fix belongs in `components/ui/wy-*.js`
2. After editing, run `npm run build:components`
