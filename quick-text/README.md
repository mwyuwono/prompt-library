...
- **Write path**: direct call into the private KeyboardServices XPC client (`_KSTextReplacementClientStore addEntries:removeEntries:withCompletionHandler:`, loaded via `dlopen`/`NSClassFromString` — the same path System Settings used on macOS 26 to talk to `keyboardservicesd`). An update must be one add-new/remove-old transaction; add-only creates a duplicate shortcut rather than replacing the prior entry.
- **Read path**: `readSystemReplacements()` prefers a read-only query of `~/Library/KeyboardServices/TextReplacements.db` because the global preferences projection can lag on macOS 27, then falls back to `NSUserDictionaryReplacementItems` from the **any-host** global preferences domain (`UserDefaults.standard.array(forKey:)`). `CFPreferencesCopyValue` with `kCFPreferencesCurrentHost` always returns `nil` for this key and silently produces an empty array (a real bug that shipped briefly and made every diff look like "add everything," see the 2026-07-09 fix in `TextReplacementSync.swift`).
- **macOS 27 compatibility boundary (verified 2026-07-28):** KeyboardServices XPC can return success and persist an entry in `~/Library/KeyboardServices/TextReplacements.db` without publishing it to the active `NSUserDictionaryReplacementItems` projection. Such an entry exists but does not expand when typed. Quick Text now reads the database read-only for accurate diffing, verifies both the database and a fresh-process preferences projection, and reports manual fallback unless both layers agree. Never call database-only presence a successful sync.
- **Runtime acceptance check:** after a sync-path change, type the shortcut plus a space in a temporary unsaved TextEdit document. Source inspection, an XPC code-0 response, database presence, and preferences presence are individually insufficient substitutes for actual expansion.
