...
- **Write path**: direct call into the private KeyboardServices XPC client (`_KSTextReplacementClientStore addEntries:removeEntries:withCompletionHandler:`, loaded via `dlopen`/`NSClassFromString` — the same path System Settings itself uses to talk to `keyboardservicesd`). Verified end-to-end on macOS 26.5.1: an XPC add/remove reliably lands in both `NSUserDictionaryReplacementItems` and `~/Library/KeyboardServices/TextReplacements.db`, and the shortcut expands/stops-expanding correctly when typed. Never write to the db or the `NSUserDictionaryReplacementItems` preference directly — the daemon treats iCloud as authoritative and silently reverts direct writes.
- **Read path**: `readSystemReplacements()` must read `NSUserDictionaryReplacementItems` from the **any-host** global preferences domain (`UserDefaults.standard.array(forKey:)`) — `CFPreferencesCopyValue` with `kCFPreferencesCurrentHost` always returns `nil` for this key and silently produces an empty array (a real bug that shipped briefly and made every diff look like "add everything," see 2026-07-09 fix in `TextReplacementSync.swift`).
- **Verify-after-write is unreliable from this app's own process.** A re-read of `NSUserDictionaryReplacementItems` *from the same long-running process that just performed the XPC write* does not reliably reflect that write for a long, unpredictable time (observed 60+ seconds even with polling and an explicit `CFPreferencesAppSynchronize` call first). A **fresh process** (`defaults read -g NSUserDictionaryReplacementItems`, or a new script) always sees the change correctly and immediately — this is an in-process cache/notification gap specific to this XPC path, not a real system propagation delay. Net effect: `applyDirect`'s verify step (polls up to ~10s) can spuriously time out and report "Direct sync unavailable" even though the write actually succeeded. This is safe — corpus sync-state is only persisted on a *confirmed* success, so nothing gets corrupted — but it's a known false-negative UX rough edge, not proof the write failed. **To genuinely verify this mechanism, always check from a fresh process (`defaults read -g ...` in a new shell) or by actually typing the shortcut in TextEdit — never trust the app's own success/fallback report alone.**
...
- The web component and public export never see this field — `scripts/export-public-corpus.mjs` strips `textReplacement` from every exported phrase.
