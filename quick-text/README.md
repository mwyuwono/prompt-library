...
- **Write path**: direct call into the private KeyboardServices XPC client (`_KSTextReplacementClientStore addEntries:removeEntries:withCompletionHandler:`, loaded via `dlopen`/`NSClassFromString` — the same path System Settings itself uses to talk to `keyboardservicesd`). Verified end-to-end on macOS 26.5.1: an XPC add propagates to both `NSUserDictionaryReplacementItems` and `~/Library/KeyboardServices/TextReplacements.db` within ~2s with no manual refresh, and the shortcut expands correctly when typed; an XPC remove cleans up both stores and the shortcut stops expanding. Never write to the db or the `NSUserDictionaryReplacementItems` preference directly — the daemon treats iCloud as authoritative and silently reverts direct writes. Every write is followed by a verify-after-write re-read of `NSUserDictionaryReplacementItems`; if the transaction errors, times out, or verification finds a mismatch, the sync reports failure and falls back to on-screen manual instructions (add/change/delete lines) plus a button to open System Settings > Keyboard > Text Replacements directly — **plist drag-import is confirmed broken on macOS 26** (shows drag feedback, imports nothing), so that path was intentionally not built.
