...
### Planned: reusable variable library (not yet implemented)

Everything above is scoped to one phrase: an inline `{{name}}` or `{{a/b}}` placeholder only exists inside that phrase's own `value` text, and two phrases can't share one. The original ask (from an earlier Antigravity session — see git history on this file and on `corpus/quick-text.json` around commit `16c28f1`, which added an inert `variables: [{id, name, options}]` stub to the corpus that nothing ever read) was for **named variables decoupled from any one phrase**, reusable across many phrases, maintained in their own library, with a controlled propagation story when a shared variable is edited. That part is still fully outstanding. Confirmed direction for when it's built:

- **Syntax distinguishes inline vs. library by an `@` sigil**: `{{name}}` and `{{a/b}}` keep meaning exactly what they mean today — a one-off, phrase-local placeholder, never touching the library. `{{@name}}` means "resolve `name` against the shared library" instead. Both forms are permanent, coexisting syntaxes — there's no migration step and no plan to deprecate inline placeholders once the library ships.
- **Corpus schema**: a real top-level `variables` array (replacing the old inert stub), each entry `{ id, name, type: "text" | "choice", options?: string[] }` — `id` is stable and never reused; `name` is the human-facing key authors type inside `{{@name}}` and must be unique (case-insensitive) among library entries; `options` is required when `type` is `"choice"`.
- **Resolution**: at render time, `{{@name}}` looks up `variables` by `name` (case-insensitive, trimmed) and renders/fills exactly like today's inline chips — free-text popover for `"text"`, choice buttons for `"choice"` — just sourced from the library entry instead of parsed inline text. A dangling reference (renamed or deleted library entry) renders as a visually distinct "unresolved" chip and copies through as the literal `{{@name}}` text, same fallback precedent as an unfilled placeholder today.
- **Authoring flow**: the Value field gets an "Insert variable" affordance offering (a) insert an existing library variable at the cursor by name, or (b) create a new library variable (name, type, options) and insert it in one step. Typing `{{name}}`/`{{a/b}}` by hand for a one-off still needs no library interaction at all. A separate "Variables Library" admin surface (new toolbar button, alongside Settings) lists every library variable, lets you edit or delete each, and shows how many phrases currently reference it (a live reverse-index scan for `{{@name}}` across `phrases[].value`, not a stored back-reference list).
- **Edit propagation** (confirmed spec): saving a change to an existing library variable's `name`, `type`, or `options` prompts, only when ≥1 phrase currently references it, with two choices:
  - **Update all** — mutate the existing entry in place (same `id`); every `{{@name}}` reference picks up the new options/type immediately. If `name` itself changed, this also rewrites `{{@oldName}}` to `{{@newName}}` in every referencing phrase's `value`, since otherwise the rename silently breaks all of them.
  - **Fork** — create a new entry with a new `id` and a new name chosen by the admin (can't collide with the original or any other existing name). **Phrases that used the old variable stay pointing at the old, unforked entry — nothing about them changes.** The fork starts referenced by nothing; the admin manually inserts `{{@newName}}` into whichever phrases should adopt it going forward.
  - Deleting a library variable that's still referenced should warn how many phrases reference it before deleting; afterward those phrases keep their literal `{{@name}}` text, which now resolves as "unresolved" rather than silently reverting to a plain inline placeholder.
