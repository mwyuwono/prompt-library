...
Use an optional prompt-level `relatedPromptIds` array to link adjacent workflows. Each value must be a unique, non-self prompt ID that exists in `prompts.json`. The public modal renders these as related-prompt buttons; there is no admin authoring control in this pass.
...
`Backup: <ISO timestamp>` commits are manual — the admin UI's "Backup now" button (`admin.js` `runBackupNow` -> `POST /api/backup/run` in `server.js`). There is no timer or scheduler, so clusters of them are a working session, not a runaway loop.

`rb-fabric-collection` carries an Ignored Build Step (`git diff --quiet HEAD^ HEAD -- .`), so pushes that touch only prompt data skip its build and land as CANCELED deployments. That is expected; do not treat those as failures.
...
**Storage is driven by the committed tree, not `.vercelignore`.** Both `prompts` and `rb-fabric-collection` are Git-linked and clone this entire repository on every deployment. `.vercelignore` keeps bulk out of the *served* output but not out of *stored* bytes, so per-deployment cost tracks `git ls-files`, not the build. Measure with `git ls-files -z | xargs -0 du -ck | tail -1` before assuming a deployment is small. Nothing gitignored is safe by assumption either: `rb-fabric-collection/dist/` was listed in `rb-fabric-collection/.gitignore` yet stayed tracked because it was committed before that rule, costing ~107 MB per deployment until it was untracked (`cb6dbc9`, tree 251 MB -> 23.6 MB). `dist/` is regenerated by `npm run build`; never re-commit it.
